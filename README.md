...
(default ref for lines that omit `@ref`), and `GH_COPILOT_INSTRUCTIONS_NO_VSCODE` (set to opt out of
installing into VS Code's prompts directory; see [VS Code](#vs-code)).
...
- **Opt out** by setting `GH_COPILOT_INSTRUCTIONS_NO_VSCODE=1`, and the tool leaves VS Code's
  directories untouched (it still installs to `~/.copilot/instructions/`).
