...
gh copilot-instructions doctor [--json]                                     # run health checks and suggest fixes
```
...
- **`doctor`** runs read-only health checks and prints a table of `STATUS` / `CHECK` / `NOTE` rows —
  covering whether the extension itself is up to date, GitHub auth, whether your sources are
  configured and reachable, the install directory, pulled files (missing or orphaned), `applyTo`
  frontmatter, the VS Code mirror, auto-pull, and more. The `CHECK` column is a fixed label and the
  `NOTE` carries the dynamic reading or the fix; checks that don't apply to this machine are shown
  dimmed. Some checks call GitHub (verifying your token, reachability, available updates, and the
  latest release). It exits non-zero if any check fails, so it works in scripts and CI; use `--json`
...
`gist.github.com` URL work; in the source list a gist shows as `<owner>/gist:<id>`:
